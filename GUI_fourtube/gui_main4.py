...
class GUI_App(ttk.Frame):
    ...
    def create_widgets(self,):
        ...
        self.var_scale21.set(7.2) #10.7) #9)
        ...
        self.var_scale22.set(2.1) #0.7) #8)
        ...
        self.var_scale23.set(6.0) #10.7) #5.6)
        ...
        gyou=gyou+1
        self.label24= ttk.Label(self.frame2, text="Length 4 [cm]")
        self.var_scale24 = tk.DoubleVar()
        self.var_scale24.set(3.6) #10.7) #5.6)
        self.scale24 = tk.Scale(self.frame2,variable=self.var_scale24,orient=tk.HORIZONTAL,tickinterval=DEF_itv1, from_=1,to=DEF_to1,resolution=0.1,length=200,)
        self.label24.grid(row=gyou, column=0)
        self.scale24.grid(row=gyou, column=1)
        ...
        self.var_scale31.set(2.2) #0.5) # 1)
        ...
        self.var_scale33.set(23.9) #20) #3)
        ...
        gyou=gyou+1
        self.label34= ttk.Label(self.frame3, text="Area 4 [cm^2]")
        self.var_scale34 = tk.DoubleVar()
        self.var_scale34.set(35.8) #20) #3)
        self.scale34 = tk.Scale(self.frame3,variable=self.var_scale34,orient=tk.HORIZONTAL,tickinterval=DEF_itv3, from_=0.1,to=DEF_to3,resolution=0.1,length=200,)
        self.label34.grid(row=gyou, column=0)
        self.scale34.grid(row=gyou, column=1)
    ...
    def button1_clicked(self,):
        ...
        # insatnce
        self.tube  =  Class_FourTube(L1, L2, L3, L4, A1, A2, A3, A4, sampling_rate=sampling_rate,A440=A440,octave=octave,resolution=resolution)
        if self.linear_scale_mode:
            self.amp1, self.freq=self.tube.H1_linear()
        else:
            self.amp1, self.freq=self.tube.H1()
        
        self.peaks=self.tube.get_peaks(self.amp1)
    ...
    def make_draw(self,):
        ...
        if self.amp1 is not None:
            ...
            for i in self.peaks:
                ...
                plt.text(self.freq[i], self.amp1[i], str(int(self.tube.f_list[i])))
        ...
        if file_name:
            ...
            try:
                with open(file_name, 'rt') as f:
                    ...
                    if len(lines0) == int(self.num_of_tube * 2):
                        self.scale21.set( float(lines0[0]) ) # L1
                        self.scale22.set( float(lines0[1]) ) # L2
                        self.scale23.set( float(lines0[2]) ) # L3
                        self.scale24.set( float(lines0[3]) ) # L4
                        
                        self.scale31.set( float(lines0[4]) ) # A1
                        self.scale32.set( float(lines0[5]) ) # A2
                        self.scale33.set( float(lines0[6]) ) # A3
                        self.scale34.set( float(lines0[7]) ) # A4
                    else:
                        print ('error: number of tube is mismatch. cannot set. ', self.num_of_tube)
    ...
    def save_wav_button1_clicked(self,):
        ...
        if file_name:
            ...
            # insatnce
            tube  =  Class_FourTube(L1, L2, L3, L4, A1, A2, A3, A4,sampling_rate=sampling_rate, A440=A440,octave=octave,resolution=resolution)
            ...
            #
            yg_repeat=glo.make_N_repeat(repeat_num=50) # input source of tube model
